...
DEFAULT_EMBEDDING_MODEL = "distilbert-base-uncased"
...
def score_self_awareness_response(
    *,
    ...
    similarity_backend: str = "embedding",
...
def rank_reference_similarities(
    ...
    *,
    ...
    corpus = [candidate_text, *references.values()]
    tokenized_docs = [_tokenize(text) for text in corpus]
    document_frequency = Counter()
    for tokens in tokenized_docs:
        document_frequency.update(set(tokens))

    doc_count = len(tokenized_docs)
    candidate_vector = _tfidf_vector(tokenized_docs[0], document_frequency, doc_count)
    scored: list[dict[str, Any]] = []
    for label, text in references.items():
        vector = _tfidf_vector(_tokenize(text), document_frequency, doc_count)
        scored.append(
            {
                "label": label,
                "similarity": round(_cosine_similarity(candidate_vector, vector), 4),
            }
        )
...
def active_persona_for_condition(condition: str, persona_a: str, persona_b: str) -> str:
    return persona_a if condition == "A_only" else persona_b
...
def _tfidf_vector(tokens: list[str], document_frequency: Counter[str], doc_count: int) -> dict[str, float]:
